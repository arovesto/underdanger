...
from data.res import not_flyable, proper_warrior
from code.geometry.direction import DIRS, anti_dir
from code.geometry.geometry import distance
...
class Spear(Weapon):
    ...
    def __init__(self, name="копье", damage=6, cost=2, range_=3):
        ...
        self.cost = cost
        self.name = name
        self.description = "Копье бьет на {} клеток нанося {} за {} очков. То что надо рыцарю".format(range_, damage,
                                                                                                      cost)
...
class Whip(Weapon):
    ...
    def use(self, direction):
        ...
        while len(queue) > 0:
            ...
            queue.extend([d.go(a) for d in dirs if
                          distance(d.go(a), self.user.position) <= self.range_ and d.go(a) not in visited])
