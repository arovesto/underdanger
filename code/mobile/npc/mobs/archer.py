...
from data.res import names, not_flyable, walkable_for_monsters
from code.geometry.direction import DIRS, anti_dir
from code.mobile.npc.npc import Npc
from code.equipment.weapons import archer_bow, better_archer_bow
from code.equipment.items import small_heal_potion, glasses, money_bag, bag, big_heal_potion
...
class Archer(Npc):
    ...
    def start_equip(self):
        self.drop = probability_range(
            {0.1: small_heal_potion(), 0.2: glasses(), 0.5: money_bag()(random.randrange(5, 15)),
             0.101: bag()(money_bag()(random.randrange(25, 35)), glasses(), big_heal_potion(), archer_bow(),
                          name='сумка лучника')})
        return self
