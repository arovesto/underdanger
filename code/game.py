...
from board import generate_game_board
from res import win_words, small_controls

from geometry import square, merge
...
class Game:
    ...
    def run_checks(self):
        ...
        if self.who_action == 'игрок' and self.action_player.name == self.names[0] and self.action_player.ap == self.action_player.max_ap:
    ...
    def run_mech(self):
        ...
        not_player_characters = [m for m in self.world.mobs_near(merge([square(pos, self.treat_radius * 2)
                                                        for pos in self.world.players])).values() if m.kind != 'игрок']
